...
public class OtherUserScene2: SKScene, SKPhysicsContactDelegate {
    ...
    weak var myPlayer:OtherUserSongVC2?
...
//        let circle = SKShapeNode(circleOfRadius: size.width / 14)
    ...
    func ajouterRond(seconde:CGFloat, colonne:Int, noteNumber:Int){
        ...
        let i1 = noteNumber - globalVar.songFirebase.songPourFile.rootNote
        let isDiese = myPlayer!.noteLine[(i1+1200) % 12]
        var quelleLigne = i1 / 12
        if quelleLigne >= 3{
            quelleLigne = 2
