...
//    func numberOfItemsInDropDown(_ dropDown: DropDown) -> Int
//    @objc optional func dropDown(_ dropDown: DropDown, titleForItem item: Int) -> String
    @objc optional func dropDown(_ dropDown: DropDown, didSelectItem item: Int)
}
...
open class DropDown : UITextField{
    ...
    var myObserver1 : Any? = nil
    var myObserver2 : Any? = nil
    public weak var delegateYoyo: DropDownDelegate? = nil
    ...
    @objc private func notifFunction1(notification: NSNotification){
    ...
    @objc private func notifFunction2(notification: NSNotification){
        if self.isFirstResponder{
    ...
    func setupUI () {
        ...
        if isSearchEnable && handleKeyboard{
            myObserver1 = NotificationCenter.default.addObserver(self, selector: #selector(notifFunction1), name: UIResponder.keyboardWillShowNotification, object: nil)
            myObserver2 = NotificationCenter.default.addObserver(self, selector: #selector(notifFunction2), name: UIResponder.keyboardWillHideNotification, object: nil)
        }
    }
    
    deinit {
        if myObserver1 != nil{
            NotificationCenter.default.removeObserver(myObserver1!)
        }
        if myObserver2 != nil {
            NotificationCenter.default.removeObserver(myObserver2!)
        }
        NotificationCenter.default.removeObserver(self)
        for recognizer in backgroundView.gestureRecognizers ?? [] {
            backgroundView.removeGestureRecognizer(recognizer)
        }
        for recognizer in self.gestureRecognizers ?? [] {
            self.removeGestureRecognizer(recognizer)
        }
        for recognizer in rightView?.gestureRecognizers ?? [] {
            self.rightView?.removeGestureRecognizer(recognizer)
        }
        didSelectCompletion = {selectedText, index , id  in }
        TableWillAppearCompletion = { }
        TableDidAppearCompletion = { }
        TableWillDisappearCompletion = { }
        TableDidDisappearCompletion = { }
    ...
    public func showList() {
        if parentController == nil{
            parentController = getParentViewController()
        }
    ...
    public func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if let selected = optionArray.firstIndex(where: {$0 == selectedText}) {
            if let id = optionIds?[selected] {
                self.delegateYoyo?.dropDown?(self, didSelectItem: selected)
